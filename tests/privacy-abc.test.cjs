...
const englishConsent = normalize(`
  ...
  Version 2 — 28 July 2026
  Operator: IP Demkin Danila (Snow Bat). Privacy contact: snowbatstudio@gmail.com.
  Purposes. Creation and protection of a pseudonymous game account; automatic sign-in on this installation; cloud synchronization and restoration of progress; operation of online features; support, abuse prevention and security.
  ...
  Processors and regions. Snow Bat backend services (snowbatstudio.ru) store the account and save. Cloudflare may route and protect requests through global infrastructure. Online multiplayer may use Photon as described in the Privacy Policy. Depending on routing, data may be processed in Russia, the EEA, the United States and other provider regions.
  ...
  Your choice and rights. You may request access, correction, portability, restriction or deletion and may withdraw this consent by contacting snowbatstudio@gmail.com. Withdrawal does not affect processing already performed. Refusal or withdrawal disables the cloud account and online features; local single-player play remains available.
  Age category and minors. Before consent, the game asks only whether the user is under 14 or aged 14 or older; it does not request or store a date of birth. Users under 14 cannot create or use a cloud account, online modes or player chat and must choose local play without cloud. Users aged 14 or older may proceed only if they have also reached the applicable digital-consent age in their country; otherwise they must play without cloud. The age category can be corrected in Settings → Gameplay; changing it invalidates the current consent receipt. Changing the category to under 14 disables access but does not itself delete previously stored account data; deletion may be requested through the privacy contact.
  This consent is only for the cloud account and required online operation. It is not consent to advertising, analytics, marketing or sale of data. Those purposes require separate controls where applicable.
...
const russianConsent = normalize(`
  ...
  Версия 2 — 28 июля 2026 года
  Оператор: ИП Демкин Данила (Snow Bat). Контакт по вопросам данных: snowbatstudio@gmail.com.
  Цели. Создание и защита псевдонимного игрового аккаунта; автоматический вход на этой установке; синхронизация и восстановление прогресса; работа сетевых функций; поддержка, предотвращение нарушений и обеспечение безопасности.
  ...
  Обработчики и регионы. Сервисы Snow Bat (snowbatstudio.ru) хранят аккаунт и сохранение. Cloudflare может маршрутизировать и защищать запросы через глобальную инфраструктуру. Сетевой режим может использовать Photon, как описано в Политике конфиденциальности. В зависимости от маршрута данные могут обрабатываться в России, ЕЭЗ, США и иных регионах провайдеров.
  ...
  Ваш выбор и права. Вы можете запросить доступ, исправление, перенос, ограничение или удаление и отозвать согласие через snowbatstudio@gmail.com. Отзыв не отменяет уже выполненную обработку. Отказ или отзыв отключает облачный аккаунт и сетевые функции; локальная одиночная игра остаётся доступна.
  Возрастная категория и несовершеннолетние. Перед согласием игра спрашивает только, младше ли пользователь 14 лет либо ему 14 лет или больше; дата рождения не запрашивается и не сохраняется. Пользователь младше 14 лет не может создать или использовать облачный аккаунт, сетевые режимы и чат игроков и должен выбрать локальную игру без облака. Пользователь 14 лет или старше может продолжить, только если он также достиг применимого в своей стране возраста цифрового согласия; иначе он должен играть без облака. Возрастную категорию можно исправить в Настройки → Gameplay; при изменении текущая квитанция согласия аннулируется. Изменение категории на «младше 14» отключает доступ, но само по себе не удаляет ранее сохранённые данные аккаунта; запросить удаление можно через контакт по вопросам данных.
  Это согласие относится только к облачному аккаунту и необходимой работе сетевых функций. Оно не является согласием на рекламу, аналитику, маркетинг или продажу данных. Для таких целей при необходимости используются отдельные настройки.
...
test("privacy_abc documents the minimal age gate and under-14 restrictions", () => {
  ...
  assert.match(text, /Users under 14 cannot create or use a cloud account, online modes or player chat/i);
  assert.match(text, /corrected in Settings → Gameplay/i);
  assert.match(text, /дата рождения не запрашивается и не сохраняется/i);
  assert.match(text, /Пользователь младше 14 лет не может создать или использовать облачный аккаунт, сетевые режимы и чат игроков/i);
});
